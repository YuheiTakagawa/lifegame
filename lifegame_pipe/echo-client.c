...
extern int echo_client_one(char *server, int portno, char *path);
extern int echo_send_request(FILE *out, char *path);
...
int main(int argc, char *argv[])
{
	...
	if(argc != 4)
	{
		fprintf(stderr,"Usage: %s host port pass\n",argv[0]);
		exit(-1);
	}
	...
	path = argv[3];
	err = echo_client_one(server, portno, path);
	return (err);
}

#define BUFFERSIZE 1024
int echo_client_one(char *server, int portno, char *path)
{
	int sock;
	FILE *in, *out;
	...
	res = echo_send_request(out, path);
	if(res < 0)
	{
...
int echo_send_request(FILE *out, char *path)
{
